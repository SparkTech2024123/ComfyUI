...
# for Kontext Omni Styles processing in ComfyUI workflows
#
...
# - Load balancing across multiple ComfyUI servers (ports 8261-8265)
...
def select_best_server(servers=None):
    ...
    print("=== Checking ComfyUI servers status (8261-8265) ===")
    available_servers = []
...
def modify_workflow_for_shared_memory_input(workflow, shm_data, style_description=None):
    """
    ...
    Args:
        ...
        style_description: 风格描述文本 (默认None，保持原值)
    Returns:
        修改后的工作流字典
    """
    # 创建工作流副本
    ...
    # 1. 替换25号LoadImage节点为LoadImageSharedMemory节点
    load_image_node_id = "25"
    if load_image_node_id in modified_workflow:
        ...
        print("Warning: LoadImage node (25) not found in workflow")
    
    # 2. 更新26号节点的风格描述
    if style_description is not None and "26" in modified_workflow:
        modified_workflow["26"]["inputs"]["text"] = style_description
        print(f"Updated style description: {style_description}")
    
    # 3. 替换32号PreviewImage节点为SaveImageSharedMemory节点
    preview_node_id = "32"
    if preview_node_id in modified_workflow:
        ...
        print("Warning: PreviewImage node (32) not found in workflow")
    
    return modified_workflow

def create_kontext_omni_styles_workflow_with_shared_memory(image_array, style_description=None):
    """
    从A-kontext-omni-styles-api.json加载工作流并修改为使用LoadImageSharedMemory节点处理numpy数组输入
    Args:
        image_array: 输入图像的numpy数组 (h, w, 3) RGB格式
    ...
    if not os.path.exists(workflow_path):
        workflow_path = os.path.join(os.path.dirname(current_dir), "user", "default", "workflows", "A-kontext-omni-styles-api.json")
    
    # 再次检查文件是否存在
    if not os.path.exists(workflow_path):
        print(f"Workflow file not found at: {workflow_path}")
        print("Please ensure A-kontext-omni-styles-api.json exists in the correct location")
    ...
    modified_workflow = modify_workflow_for_shared_memory_input(
        original_workflow, shm_data, style_description
    )
    
    return modified_workflow, shm_obj

def process_image_kontext_omni_styles_numpy_shared_memory(image_array, style_description=None,
                                                        server_address=None, comfy_api_key=None):
    """
    使用Kontext Omni Styles处理numpy图像数组（共享内存版本）
    Args:
        image_array: 输入图像的numpy数组 (h, w, 3) RGB格式
        style_description: 风格描述文本
    ...
    print(f"Parameters: style={style_description}")
    ...
    try:
        ...
        workflow, shm_obj = create_kontext_omni_styles_workflow_with_shared_memory(
            image_array, style_description
        )
        ...
        print(f"Executing ComfyUI Kontext Omni Styles workflow on {server_address}...")
        ...
        if shared_memory_info:
            ...
            print(f"\n=== Kontext Omni Styles Processing Time Summary (Shared Memory Output) ===")
...
# 主要接口
def comfyui_kontext_omni_styles_process(image_array, style_description=None, 
                                       comfy_api_key=None, **kwargs):
    """
    Kontext Omni Styles处理接口（使用共享内存和负载均衡）
    Args:
        image_array: 输入图像的numpy数组 (h, w, 3) RGB格式
        style_description: 风格描述文本
        comfy_api_key: API密钥用于认证
    ...
    return process_image_kontext_omni_styles_numpy_shared_memory(
        image_array, style_description, 
...
if __name__ == "__main__":
    # 示例：使用numpy数组处理图像
    try:
        print("=== Kontext Omni Styles generation with shared memory input and load balancing ===")
        ...
        # 请替换为实际的输入图像路径
        input_image_path = "input/Amiaofu3.png"
        ...
        cv2.imwrite("kontext_omni_styles_input.png", cv2.cvtColor(input_image_array, cv2.COLOR_RGB2BGR))
        ...
        # 使用numpy数组处理图像
        result_image = process_image_kontext_omni_styles_numpy_shared_memory(
            image_array=input_image_array,
            style_description=style_templates["origami"],
        ...
        # 保存结果（RGBA格式直接保存）
        output_path = "kontext_omni_styles_result_origami.png"
        Image.fromarray(result_image, 'RGBA').save(output_path)
        print(f"Result saved as: {output_path}")
        
        # 测试不同的风格
        print("\n--- Testing different style combinations ---")
        result2 = process_image_kontext_omni_styles_numpy_shared_memory(
            image_array=input_image_array,
            style_description=style_templates["watercolor"],
            comfy_api_key=api_key
        )
        
        output_path2 = "kontext_omni_styles_result_watercolor.png"
        ...
        result = comfyui_kontext_omni_styles_process(
            input_image_array,
            style_description=style_templates["anime"],
            comfy_api_key=api_key
        )
        ...
        print("✓ Load balancing across multiple ComfyUI servers (8261-8265)")
        print("✓ Automatic server selection based on queue status and VRAM usage")
        print("✓ Support for style description control")
