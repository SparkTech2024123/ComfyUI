...
def process_image_remove_bg_numpy_shared_memory(image_array, model="RMBG-2.0", sensitivity=1, process_res=1024, 
    ...
    try:
        ...
        print("Executing ComfyUI remove background workflow...")
        ...
        if 'save_image_websocket_node' in images:
            ...
            print(f"\n=== Remove Background Processing Time Summary (Shared Memory) ===")
...
def comfyui_remove_bg_process(image_array, **kwargs):
    """
    Remove Background处理接口（使用共享内存）
    Args:
        image_array: 输入图像的numpy数组 (h, w, 3) RGB格式
        **kwargs: 其他参数 (model, sensitivity, process_res, etc.)
            Returns:
        numpy数组，RGBA或RGB格式
    """
